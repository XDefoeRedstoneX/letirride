...
<div class="ridly-products"
    ...
    <div class="page-inner">
        ...
        <div class="page-header">
            <a href="{{ route('home') }}" class="catalog-back">← BROWSE BY TYPE</a>
            <h1 class="page-title">ALL <span class="gold">PRODUCTS</span></h1>
            <p class="page-sub">THE FULL RIDLY CATALOG — SEARCH, FILTER, AND BUY</p>
        </div>
...
/* Hub group key → the categories it covers (mirrors RIDLY_HUB_GROUPS on the
   home hub). Used to honour ?group= deep links from each section's VIEW ALL. */
...
function productsPage(initialProducts, initialFavorites, isAuthenticated, csrfToken) {
    return {
        ...
        init() {
            ...
            // Deep link from the hub search box: ?search=keyword
            const search = urlParams.get('search');
            if (search) {
                this.search = search;
            }

            // Deep link from a hub section's VIEW ALL: ?group=subs scopes the
            // catalog to that hub group's categories.
            const group = urlParams.get('group');
            if (group && RIDLY_GROUP_CATS[group]) {
                this.groupCats = RIDLY_GROUP_CATS[group];
            }

            // Deep link from the browse hub: ?brand=Steam pre-selects that brand.
            const brand = urlParams.get('brand');
            if (brand && this.products.some(p => (p.subcategory || 'Other') === brand)) {
                this.selectedBrand = brand;
            }

            // Deep link to a specific product's buy modal: ?buy=PRODUCT_ID
        ...
        // Categories visible in the grouped catalog (respects pill + ?group scope).
        ...
        // A category is in scope when it passes the active pill AND the ?group filter.
        _inScope(cat) {
        ...
        selectBrand(name) {
            ...
            // Scroll down to the product list so the change is visible.
            if (this.selectedBrand) {
                this.$nextTick(() => {
