            ...
            <div class="bg-card border border-border rounded-2xl p-6 space-y-6">
                ...
                <form class="space-y-4" x-ref="profileForm" @submit.prevent="submitProfile" method="POST" action="{{ route('updateProfile') }}">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" value="{{ Auth::check() ? Auth::user()->name : '' }}" x-model="profileName" class="w-full px-4 py-2 bg-background border border-border rounded-xl focus:ring-2 focus:ring-primary/50 outline-none transition-all">
                    </div>

                    <div class="space-y-2">
                        <label class="text-sm font-medium">Email Address</label>
                        <input type="email" name="email" value="{{ Auth::check() ? Auth::user()->email : '' }}" disabled class="w-full px-4 py-2 bg-muted border border-border rounded-xl cursor-not-allowed opacity-70">
