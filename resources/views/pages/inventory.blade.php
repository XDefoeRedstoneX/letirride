<x-app-layout>
    <style>
        /* Let the badges + date share the row and wrap instead of pushing the
           date outside the card on narrow screens. */
        .inv-head   { flex-wrap: wrap; gap: 6px; }
        .inv-badges { flex-wrap: wrap; min-width: 0; }

        /* Compact inventory cards on mobile so content stays inside the box. */
        @media (max-width: 639px) {
            .inv-item  { padding: 12px !important; gap: 10px !important; }
            .inv-thumb { width: 48px !important; height: 48px !important; padding: 8px !important; }
            .inv-date  { font-size: 5px !important; }
        }
    </style>
    <div class="px-page">
        <div class="px-page-inner space-y-8"
            ...
            <div style="display:flex;flex-direction:column;gap:16px;" class="md:flex-row md:items-center md:justify-between">
                ...
                <div style="position:relative;width:100%;max-width:320px;">
                    <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="square" style="position:absolute;left:12px;top:50%;transform:translateY(-50%);color:var(--muted-foreground);" class="pixel-render"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <template x-for="(item, index) in filteredItems" :key="index">
                    <div @click="openDetail(item)" class="px-card inv-item" style="padding:20px;display:flex;gap:16px;align-items:center;cursor:pointer;">
                        <div class="inv-thumb" style="width:64px;height:64px;background:var(--muted);border:2px solid var(--border);display:flex;align-items:center;justify-content:center;padding:12px;flex-shrink:0;">
                            <img :src="item.image" class="w-full h-full object-contain pixel-render" />
                        </div>
                        <div style="flex:1;min-width:0;display:flex;flex-direction:column;gap:6px;">
                            <div class="inv-head" style="display:flex;align-items:center;justify-content:space-between;">
                                <div class="inv-badges" style="display:flex;align-items:center;gap:6px;">
                                ...
                                <span class="inv-date" style="font-family:var(--px);font-size:6px;color:var(--muted-foreground);letter-spacing:0.08em;white-space:nowrap;flex-shrink:0;" x-text="item.date"></span>
                            </div>
                            <h3 style="font-family:var(--font-sans);font-size:14px;font-weight:800;color:var(--foreground);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" x-text="item.name"></h3>
                            <template x-if="item.item_type === 'voucher_key' || item.item_type === 'discount'">
                                <div style="display:flex;align-items:center;gap:8px;">
                                    <span class="px-badge px-badge-blue">CODE</span>
                                    <span style="font-family:var(--px);font-size:5px;color:var(--muted-foreground);">TAP TO REVEAL</span>
                                </div>
                            </template>
                            <template x-if="item.item_type === 'direct_topup'">
                                <div style="display:flex;align-items:center;gap:8px;">
                                    ...
                                    <span style="font-family:var(--px);font-size:5px;color:var(--muted-foreground);">TAP FOR DETAILS</span>
                                </div>
                            </template>
                        </div>
                    </div>
                ...
                <div class="px-modal-box" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" @click.stop>
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;">
                        <div style="display:flex;align-items:center;gap:12px;">
                            <div style="width:40px;height:40px;background:var(--muted);border:2px solid var(--border);display:flex;align-items:center;justify-content:center;padding:6px;">
                                <img :src="selectedItem?.image" class="w-full h-full object-contain pixel-render" />
                            </div>
                            <div>
                                <h3 style="font-family:var(--font-sans);font-size:14px;font-weight:800;color:var(--foreground);" x-text="selectedItem?.name"></h3>
                    ...
                    <template x-if="selectedItem && (selectedItem.item_type === 'voucher_key' || selectedItem.item_type === 'discount')">
                        <div style="display:flex;flex-direction:column;gap:14px;">
                            <div style="background:var(--muted);border:2px solid var(--border);padding:16px;text-align:center;">
                                <p style="font-family:var(--px);font-size:6px;letter-spacing:0.12em;color:var(--muted-foreground);margin-bottom:8px;">REDEEM CODE</p>
                            ...
                            <p style="font-family:var(--px);font-size:6px;color:var(--muted-foreground);text-align:center;letter-spacing:0.1em;" x-text="selectedItem.item_type === 'discount' ? 'EXPIRES: ' + selectedItem.date : 'PURCHASED: ' + selectedItem.date"></p>
                        </div>
                    </template>
                    <template x-if="selectedItem && selectedItem.item_type === 'discount_redeemed'">
                        <div style="display:flex;flex-direction:column;gap:14px;">
                            <div style="background:rgba(148,163,184,0.08);border:2px solid rgba(148,163,184,0.25);padding:16px;text-align:center;">
                                ...
                                <p style="font-family:var(--font-sans);font-size:12px;color:var(--muted-foreground);">This voucher has been consumed and cannot be reused.</p>
                            </div>
                            <template x-if="selectedItem.redeemed_order">
                                <div style="background:var(--muted);border:2px solid var(--border);padding:14px;display:flex;justify-content:space-between;align-items:center;">
                                    <span style="font-family:var(--px);font-size:6px;color:var(--muted-foreground);letter-spacing:0.12em;">APPLIED TO ORDER</span>
                            ...
                            <p style="font-family:var(--px);font-size:6px;color:var(--muted-foreground);text-align:center;letter-spacing:0.1em;" x-text="'REDEEMED: ' + selectedItem.date"></p>
                        </div>
                    </template>
                    <template x-if="selectedItem && selectedItem.item_type === 'direct_topup'">
                        <div style="display:flex;flex-direction:column;gap:14px;">
                            <div style="padding:16px;text-align:center;border:2px solid;" :style="'border-color:' + (selectedItem.topup_status === 'sent' ? '#22c55e' : selectedItem.topup_status === 'processing' ? '#3b82f6' : '#f59e0b') + ';background:' + (selectedItem.topup_status === 'sent' ? 'rgba(34,197,94,0.1)' : selectedItem.topup_status === 'processing' ? 'rgba(59,130,246,0.1)' : 'rgba(245,158,11,0.1)')">
                                ...
                                <p style="font-family:var(--font-sans);font-size:12px;color:var(--muted-foreground);" x-text="selectedItem.topup_status === 'sent' ? 'Top-up has been sent to your account.' : (selectedItem.topup_status === 'processing' ? 'Our delivery bot is processing your top-up. This usually takes under a minute.' : 'Awaiting payment confirmation.')"></p>
                            </div>
                            <div style="background:var(--muted);border:2px solid var(--border);padding:14px;display:flex;flex-direction:column;gap:8px;">
                                <p style="font-family:var(--px);font-size:6px;letter-spacing:0.12em;color:var(--muted-foreground);">YOUR SUBMITTED CREDENTIALS</p>
                                <div style="display:flex;justify-content:space-between;"><span style="font-family:var(--px);font-size:6px;color:var(--muted-foreground);">PLAYER ID</span><span style="font-family:var(--font-sans);font-size:13px;font-weight:800;color:var(--foreground);" x-text="selectedItem.player_id || '—'"></span></div>
                                <template x-if="selectedItem.zone_id"><div style="display:flex;justify-content:space-between;"><span style="font-family:var(--px);font-size:6px;color:var(--muted-foreground);">ZONE ID</span><span style="font-family:var(--font-sans);font-size:13px;font-weight:800;color:var(--foreground);" x-text="selectedItem.zone_id"></span></div></template>
                                <template x-if="selectedItem.server_id"><div style="display:flex;justify-content:space-between;"><span style="font-family:var(--px);font-size:6px;color:var(--muted-foreground);">SERVER ID</span><span style="font-family:var(--font-sans);font-size:13px;font-weight:800;color:var(--foreground);" x-text="selectedItem.server_id"></span></div></template>
                            </div>
                            <p style="font-family:var(--px);font-size:6px;color:var(--muted-foreground);text-align:center;letter-spacing:0.1em;" x-text="'PURCHASED: ' + selectedItem.date"></p>
                        </div>
                    </template>
                </div>
            </div>
